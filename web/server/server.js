...
const anisetteUrl = (process.env.ANISETTE_URL || 'https://ani.sidestore.io').replace(/\/$/, '');
...
/*
 * SideStore-compatible Anisette V3 relay.
 * Anisette data is generated by the configured V3 service; Apple Account
 * credentials, cookies, and verification codes are never accepted here.
 * SideStore documents V3 as the preferred protocol for current releases.
 */
async function anisetteRelay(pathname, req, res) {
  const target = `${anisetteUrl}${pathname}`;
  const headers = {};
  for (const name of ['accept', 'content-type', 'user-agent', 'x-apple-i-mime-version', 'x-apple-i-client-time']) {
    const value = req.get(name);
    if (value) headers[name] = value;
  }
  const options = {method: req.method, headers, redirect: 'follow'};
...
app.get('/v1/anisette/config', (req, res) => res.json({protocol: 'anisette-v3', endpoint: `${origin}/v1/anisette/v3`}));
app.get('/v1/anisette/v3/client_info', (req, res) => anisetteRelay('/v3/client_info', req, res).catch(e => res.status(502).json({error: e.message})));
app.post('/v1/anisette/v3/provisioning', (req, res) => anisetteRelay('/v3/provisioning', req, res).catch(e => res.status(502).json({error: e.message})));
app.get('/v1/anisette/v3/health', async (req, res) => {
  try {
    const r = await fetch(`${anisetteUrl}/v3/client_info`, {method: 'GET'});
    res.status(r.ok ? 200 : 502).json({ok: r.ok, upstream: anisetteUrl, status: r.status});
  } catch (e) {
    res.status(502).json({ok: false, upstream: anisetteUrl, error: e.message});
  }
});
...
async function signIPA(input, output) {
  ...
  try {
    ...
    for (const bundle of nested) await execFileAsync('/usr/bin/codesign', ['--force', '--sign', identity, '--timestamp=none', bundle]);
...
app.post('/v1/jobs', upload.single('ipa'), async (req, res) => {
  ...
  if (!job.bundleId) { await fs.rm(job.file, {force: true}).catch(() => {}); return res.status(400).json({error: 'Bundle identifier is required'}); }
  jobs.set(job.id, job);
  try {
    job.status = 'signing';
...
app.post('/v1/auth/verify', (req, res) => res.status(410).json({error: 'Direct Apple ID password/2FA collection is not implemented. Apple authentication must remain in the authorized SideStore-compatible signing flow; Anisette V3 does not receive account credentials.'}));
...
app.get('/v1/devices', (req, res) => res.json({devices: [], mode: 'provisioned-profile', message: 'Device registration is represented by the provisioning profile installed on the authorized signing worker.'}));
app.get('/v1/manifest/:id', (req, res) => { const job = jobs.get(req.params.id); if (!job || job.status !== 'complete') return res.status(404).end(); res.type('application/xml').send(plist(job)); });
app.get('/v1/ipa/:id', (req, res) => { const job = jobs.get(req.params.id); if (!job || job.status !== 'complete' || !job.signedFile) return res.status(404).end(); res.type('application/octet-stream').download(job.signedFile, job.originalName); });
app.get('/healthz', (req, res) => res.json({ok: true, signer: adapter ? 'remote-adapter' : process.platform === 'darwin' ? 'local-macos' : 'unconfigured', anisette: anisetteUrl}));
app.use(express.static(publicRoot));
app.listen(port, () => console.log(`SideStore Web signing gateway listening on ${origin}`));

process.on('SIGTERM', async () => { for (const job of jobs.values()) { if (job.file) await fs.rm(job.file, {force: true}).catch(() => {}); if (job.signedFile) await fs.rm(job.signedFile, {force: true}).catch(() => {}); } process.exit(0); });
